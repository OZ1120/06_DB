...
SELECT C1.TABLE_NAME, CONSTRAINT_NAME, CONSTRAINT_TYPE, SEARCH_CONDITI ON
FROM USER_CONSTRAINTS C1
JOIN USER_CONS_COLUMNS C2 USING(CONSTRAINT_NAME)
WHERE C1.TABLE_NAME = 'DEPT_COPY';
...
ALTER TABLE DEPT_COPY 
MODIFY LOCATION_ID CONSTRAINT SYS_C007455 NULL; -- NULL 허용하겠따
...
-- 기본값 바꿨다고 기존값이 바뀌지 않음

SELECT * FROM DEPT_COPY;

-- LNAME컬럼 값을 모두 '대한민국' 으로 변경
UPDATE DEPT_COPY 
SET LNAME = DEFAULT ;

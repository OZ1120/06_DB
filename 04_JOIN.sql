...
-- 모든 행이 퇴종결과(RESULT SET)에 포함 되도록 하는 JOIN
-- == LEFT JOIN
...
--> 모든 경우의 수를 보고싶을 때 빼고는 직접 작상하는 경우가 없음
...
-- 		LOCATION_ID 컬럼이 존재하짖 않아 JOIN 실패
...
SELECT EMP_NAME, JOB_NAME,
	SALARY,SALARY*12 +(SALARY*12 *NVL(BONUS, 0))   연봉
FROM EMPLOYEE 
JOIN JOB USING(JOB_CODE)
...
-- SELF JOIN 사용
SELECT EMP.EMP_NAME 사원명, EMP.DEPT_CODE 부서코드 ,CO.EMP_NAME 동료이름
FROM EMPLOYEE EMP
JOIN EMPLOYEE CO ON (EMP.DEPT_CODE = CO.DEPT_CODE)
WHERE EMP.EMP_ID != CO.EMP_ID

...
/* 숫자 -> 문자열 변환시 문제 상황*/
--> 포맷에 지정된 칸 수 가 숫자 길이보다 적으면 전부 #으로 변환되어 출력
...
--> 패턴으로 인식되어 오류가 발생하기 않음!

-- 년,월,일 같은 한글 또는 날짜와 관련 없는 문자는 패턴X
--> 이런한 관련없는 문자들을 "" 감싸면 사용가능!!!
-- (""의미 : 있는 그대로 인식 )
...
FROM DUAL;
-- 문자열이 날짜를 ㅠㅛ현하는 형식이면 포맷 지정 없이 바로 변경가능
...
-- 50 이상
SELECT 
	TO_DATE('50-11-25', 'YY-MM-DD') "YY", -- 2049
...
-- NVL(컬럼명, 멀럼 값이 NULL일 경우 변경할 값)
-- 지정된 컬럼이 NULL일경우 두번째 값으로 바꾸겠다

...
SELECT EMP_ID 사번, EMP_NAME 사원명  , PHONE , SALARY , JOB_NAME,HIRE_DATE
FROM EMPLOYEE 
...
SELECT EMP_ID , EMP_NAME ,JOB_NAME, 
	CEIL(MONTHS_BETWEEN(SYSDATE, SUBSTR(EMP_NO, 1,6) ) / 12) 만나이 ,
	BONUS 
FROM EMPLOYEE 
NATURAL JOIN JOB 
WHERE EMP_NO IN (
	SELECT MIN(TO_DATE(EMP_NO , 'YYMMDD'))
	FROM EMPLOYEE 
	GROUP BY JOB_CODE 
	)
	ORDER BY HIRE_DATE ;

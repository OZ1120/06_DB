...
HAVING SUM(SALARY) =
	(SELECT MAX(SUM(SALARY))
	FROM EMPLOYEE e 		-- 서브 쿼리(17700000)
	GROUP BY DEPT_CODE);
...
-- 1) 부서별 최고 급여 조회
SELECT MAX(SALARY)
FROM EMPLOYEE 		-- 여기까지는 전체
GROUP BY DEPT_CODE ; -- 여기까지가 부서별
...
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE  
WHERE SAL_LEVEL IN(2000000, 6000000);
...
-- 3) 본인 직급의 평균 급여를 받고 있는 직원SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
SELECT EMP_ID, EMP_NAME, JOB_CODE, SALARY
FROM EMPLOYEE 
...
WHERE SALARY > (
		...
		-- MAIN 한테 전달 받은 JOB_CODE의 직급의 평균 급여 조회
);
...
-- WHERE EXITST (서브 쿼리)
 --> 서브 쿼리 결과(RESULT SET)에 행이 존재한다면 TRUE
 -- 없으면 FALSE
...
-- 2) ROWNUM : 행번호를 나타내는 가상 컬럼
 --> SELECT절 해석 되는 당시의 행 번호를 기억한다

SELECT ROWNUM, EMP_NAME
FROM EMPLOYEE 
WHERE ROWNUM <=5;

-- 3) ROWNUM 이용서 급여 상위 5명 조회
...
-- 급여 평균이 3위 안에 드는 부서의 부서코드와 부서명, 평균급여를 ;
															/* 인라인 뷰에 표시된 컬럼명 사용*/
